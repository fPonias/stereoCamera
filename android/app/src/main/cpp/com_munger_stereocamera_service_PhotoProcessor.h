/* DO NOT EDIT THIS FILE - it is machine generated */
#include <jni.h>
/* Header for class com_munger_stereocamera_service_PhotoProcessor */

#ifndef _Included_com_munger_stereocamera_service_PhotoProcessor
#define _Included_com_munger_stereocamera_service_PhotoProcessor
#ifdef __cplusplus
extern "C" {
#endif
/*
 * Class:     com_munger_stereocamera_service_PhotoProcessor
 * Method:    initN
 * Signature: (Ljava/lang/String;)V
 */
JNIEXPORT void JNICALL Java_com_munger_stereocamera_service_PhotoProcessor_initN
  (JNIEnv *, jobject, jstring);

/*
 * Class:     com_munger_stereocamera_service_PhotoProcessor
 * Method:    setProcessorType
 * Signature: (I)V
 */
JNIEXPORT void JNICALL Java_com_munger_stereocamera_service_PhotoProcessor_setProcessorType
  (JNIEnv *, jobject, jint);

/*
 * Class:     com_munger_stereocamera_service_PhotoProcessor
 * Method:    setImageN
 * Signature: (ZLjava/lang/String;IF)V
 */
JNIEXPORT void JNICALL Java_com_munger_stereocamera_service_PhotoProcessor_setImageN
  (JNIEnv *, jobject, jboolean, jstring, jint, jfloat);

/*
 * Class:     com_munger_stereocamera_service_PhotoProcessor
 * Method:    processN
 * Signature: (ZZLjava/lang/String;)V
 */
JNIEXPORT void JNICALL Java_com_munger_stereocamera_service_PhotoProcessor_processN
  (JNIEnv *, jobject, jboolean, jboolean, jstring);

/*
 * Class:     com_munger_stereocamera_service_PhotoProcessor
 * Method:    cleanUpN
 * Signature: ()V
 */
JNIEXPORT void JNICALL Java_com_munger_stereocamera_service_PhotoProcessor_cleanUpN
  (JNIEnv *, jobject);

#ifdef __cplusplus
}
#endif
#endif
