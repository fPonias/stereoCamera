...
public abstract class BaseActivity extends AppCompatActivity
{
	...
	private void requestPermissionForResult2(String permissions, PermissionResultListener listener)
	{
		...
		synchronized (lock)
		{
			...
			{
				code = requestedPermissions.get(permissions);

				if (resultListeners.containsKey(code))
				{
					ArrayList<PermissionResultListener> listeners = (ArrayList<PermissionResultListener>) resultListeners.get(code);
