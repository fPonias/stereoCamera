...
public class BluetoothSlave implements SocketCtrl {
	...
	private void doListen() {
		try {
			...
			if (ActivityCompat.checkSelfPermission(server.getParent(), Manifest.permission.BLUETOOTH_CONNECT) != PackageManager.PERMISSION_GRANTED) {
				return;
			}
			serverSocket = server.getAdapter().listenUsingRfcommWithServiceRecord(MyApplication.BT_SERVICE_NAME, BluetoothCtrl.APP_ID);

			if (serverSocket == null) {
				Log.d("stereoCamera", "null server socket returned");
				listenListener.onFailed();
				return;
			}
