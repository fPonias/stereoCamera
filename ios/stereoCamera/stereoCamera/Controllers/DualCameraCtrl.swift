...
@available(iOS 13, *)
class DualCameraCtrl: UIViewController, AVCaptureAudioDataOutputSampleBufferDelegate, AVCaptureVideoDataOutputSampleBufferDelegate{
    ...
    @IBOutlet weak var leftCameraPreview: VideoPreview!
        ...
        private func createVideoTransform() -> CGAffineTransform? {
            ...
            let backCameraTransform = CGAffineTransform.identity
            ...
            PHPhotoLibrary.requestAuthorization { status in
                if status == .authorized {
                    ...
                    PHPhotoLibrary.shared().performChanges({
                        ...
                        if !success {
                            print("\("foo") couldn't save the movie to your photo library: \(String(describing: error))")
                        } else {
                            // Clean up
                            ...
                            if let currentBackgroundRecordingID = self.backgroundRecordingID {
                                self.backgroundRecordingID = UIBackgroundTaskIdentifier.
                    ...
                    DispatchQueue.main.async {
                        ...
                        let message = NSLocalizedString("\("foo") does not have permission to access the photo library", comment: alertMessage)
                        let alertController = UIAlertController(title: "foo", message: message, preferredStyle: .alert)
                        self.present(alertController, animated: true, completion: nil)
                    }
                }
    ...
    /*
        ...
        private func createVideoSampleBufferWithPixelBuffer(_ pixelBuffer: CVPixelBuffer, presentationTime: CMTime) -> CMSampleBuffer? {
            ...
            let err = CMSampleBufferCreateForImageBuffer(kCFAllocatorDefault,
                                                         pixelBuffer,
                                                         true,
                                                         nil,
                                                         nil,
                                                         videoTrackSourceFormatDescription,
                                                         &timingInfo,
                                                         &sampleBuffer)
            if sampleBuffer == nil {
